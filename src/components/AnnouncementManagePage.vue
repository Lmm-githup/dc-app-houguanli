<template>
  ...
  <div v-else>
    <BaseSearchForm @search="search">
      ...
      <el-button slot="buttons" type="primary" @click="addTraning">添加通告</el-button>
    </BaseSearchForm>
...
import Child from "@/components/AnnouncementManageChildPage";
export default {
  created() {
    this.search();
  },
        ...
        .then(({ code, msg, data }) => {
          ...
          this.tableData = list.map(v => {
            return [
              ...
              v.status==1 ? ["发布","修改","预览","删除"] : ["取消发布","修改","预览"],
              v
            ];
          });
        });
