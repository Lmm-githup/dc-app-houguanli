<template>
  <div>
    <BaseBreadcrumb v-if="!currentId" :navList="[['通告管理'],['新建通告']]" @click="$emit('destroy')"/>
    <BaseBreadcrumb v-if="currentId" :navList="[['通告管理'],['通告详情']]" @click="$emit('destroy')"/>
    <div style="max-width:680px;">
      <el-form 
        ref="validateInfoRef"
        :rules="dialogRulesModify" 
        :model="contentForm" label-width="80px" style="margin-top:30px;" size="mini">
          <el-form-item label="通告类型" prop="category">
...
export default {
  ...
  methods: {
    ...
    initCategory(){
       commonService.lov('ANNOUNCEMENT').then(({code,msg,data}) => {
           if(code == 200){
